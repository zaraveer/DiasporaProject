      ...
      $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  // changed to get_user_by_identity from email
			...
			if ($user)
			{
				...
				if(!$this->config->item('use_ci_email', 'ion_auth'))
				{
  				...
  				$mail->Host = "sahabatdiaspora.com";
  				$mail->Port = 587;
  				$mail->SMTPAuth = true;
  				$mail->Username = "admin@sahabatdiaspora.com";
  				$mail->Password = "12345";
  				$mail->setFrom('admin@sahabatdiaspora.com', 'Sahabat Diaspora');
  				$mail->addReplyTo('admin@sahabatdiaspora.com', 'Sahabat Diaspora');
		...
		else
		{
			...
			{	
				...
				$mail->SMTPDebug = 0;
  				...
  				$mail->addReplyTo('admin@sahabatdiaspora.com', 'Sahabat Diaspora');
  				$mail->addAddress($user->email, '');
  				$mail->Subject = $this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_activation_subject');
  				$mail->msgHTML($message);
